import { format } from "date-fns";
import React, { useEffect, useState } from "react";

import AppoinmentTake from "./AppoinmentTake";

const AvailableAppointment = ({ selectedDate }) => {
  const [appointmentTake, setAppoinmentTake] = useState([]);
  useEffect ( () => {
    fetch('services.json')
    .then(res => res.json())
    .then(data => setAppoinmentTake(data))
  },[])
  return (
    <div className="text-center mt-16">
      <section>
        <h1 className="text-2xl font-serif text-emerald-500">
          Available Appointment on: <p className="text-red-600"> {format(selectedDate, "PP")}{" "}</p>   
        </h1>
<div className=" grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
  {
    appointmentTake.map(appointment => <AppoinmentTake 
      appointment={appointment} key={appointment._id} 
      ></AppoinmentTake>

    )
  }

</div>


      </section>
    </div>
  );
};

export default AvailableAppointment;
